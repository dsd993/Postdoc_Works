...
int main()
{
	...
	double xv1, yv1, zv1, xv2, yv2, zv2, E2E_Dist, OPV, TotalValue, TotalValue_DotProd, InitialValue;
	...
	//Output files
	for (int id1 = 0; id1 < NumberofBonds; id1++) //This loop basically loops over each bond in the chain
	{
	id2 = id1 + 1;
	...
	while(1)
	{
		...
		TotalValue_DotProd /=  (NumberofFiles*NumberofChains); //Dividing the accumulated value by number of files and total number of chains in the system
		
		if (n == 0)
		InitialValue = TotalValue_DotProd; //Getting the initial value at t = 0 for normalization
		
		OutputResults1 << n * IntervalTime << ' ' << TotalValue << ' ' << (TotalValue_DotProd/InitialValue) << endl; //Outputting cos(theta) as well as dot product normalized by initial value at t = 0 results
		n++;
		NumberofFiles -= 1;
	}
	}
