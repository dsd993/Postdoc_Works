...
int main()
{
	...
	while(1)
	{
		...
		TotalValue /=  (NumberofFiles*NumberofChains);
		TotalValue_DotProd /=  (NumberofFiles*NumberofChains);
		
		if (n == 0)
			InitialValue = TotalValue_DotProd;
	
		OutputResults_Intra_E2EAutoCorr << n * IntervalTime << ' ' << TotalValue << ' ' << (TotalValue_DotProd/InitialValue) << endl; //Outputs
		n++;
